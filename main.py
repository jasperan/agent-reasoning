...
def run_benchmark():
    tasks = [
        ("Philosophy (CoT)", "What is the meaning of life? Answer with a mix of biological and philosophical perspectives."),
        ...
        ("Tool Use (ReAct)", "Calculate the square root of 144 times 153, then subtract the population of 'Smallville' (search for it).")
    ]
    ...
    for category, query in tasks:
        ...
        if "Philosophy" in category:
             current_agents = [StandardAgent(), CoTAgent(), SelfReflectionAgent()]
        elif "Logic" in category:
             current_agents = [StandardAgent(), ToTAgent()]
        elif "Planning" in category:
             current_agents = [StandardAgent(), DecomposedAgent()] # Decomposed is great for itineraries
